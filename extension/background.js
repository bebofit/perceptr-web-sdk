...
function scheduleIdleTask(task, timeout = 1e3) {
...
var EventBuffer = class {
  constructor(config, onFlush) {
    ...
    this.internalConfig = {
      ...
      staleBufferThreshold: config.staleThreshold ?? 6e4,
      // 1 minute default
      backoffInterval: 5e3,
      // 5 seconds initial backoff
      maxBackoffInterval: 3e5,
  ...
  async handleSessionResume() {
    if (!this.internalConfig.persistenceEnabled || typeof localStorage === "undefined") {
    ...
    const mostRecent = persistedData.reduce(
  ...
  setupUnloadHandler() {
    if (typeof window === "undefined" || this.unloadHandlerAttached || !this.internalConfig.persistenceEnabled)
      return;
  ...
  async flushStoredBuffers(storedBuffers) {
    try {
      ...
      for (const data of [...storedBuffers]) {
        ...
        try {
          ...
          const batchStartTime = useContiguousTime ? this.lastBatchEndTime : data.startTime;
          const snapshot = {
            isSessionEnded: !isSameSession,
            sessionId: data.sessionId,
            batchId: data.batchId,
            startTime: batchStartTime || data.startTime,
            // Fallback to original start time if needed
            endTime: data.endTime,
            size: data.size,
            data: data.events,
            metadata: {
              bufferCount: v4_default(),
  ...
  persistBufferData() {
    if (!this.internalConfig.persistenceEnabled || typeof localStorage === "undefined" || this.buffer.length === 0)
      return;
    try {
      let persistedData = [];
  ...
  addEvent(event) {
    ...
    const shouldAttemptFlush = (
      ...
      this.bufferSize > this.internalConfig.maxBufferSize * 0.9 || // Buffer is too old
      now - this.lastFlushTime > this.internalConfig.staleBufferThreshold
    );
    if (shouldAttemptFlush && !this.isFlushInProgress && now > this.backoffUntil) {
  ...
  async flush(isSessionEnded = false) {
    ...
    const shouldCompress = this.internalConfig.useCompression && bufferSize > this.internalConfig.compressionThreshold;
    ...
    try {
      ...
      const backoffTime = Math.min(
        this.internalConfig.backoffInterval * Math.pow(2, this.flushFailures - 1),
        this.internalConfig.maxBackoffInterval
      );
      this.backoffUntil = now + backoffTime;
      logger.error(
        `Failed to flush buffer (attempt ${this.flushFailures}). Backing off for ${backoffTime / 1e3}s until ${new Date(
          this.backoffUntil
        ).toISOString()}`
      );
      if (this.bufferSize > SEVEN_MEGABYTES * 20) {
        const eventsToKeep = Math.floor(this.buffer.length * 0.8);
        logger.debug(
          `Buffer too large after flush failures. Dropping ${this.buffer.length - eventsToKeep} oldest events`
        );
...
var ApiService = class {
  ...
  async getUploadBufferUrl(sessionId) {
    try {
      ...
      if (error instanceof AxiosError2) {
        if (error.response?.status === 400 && error.response?.data.detail === "processing already started") {
          logger.debug(
  ...
  async sendEvents(buffer) {
    const url = await this.getUploadBufferUrl(buffer.sessionId);
    if (!url) {
      return;
    }
    await axios_default.put(url, buffer, {
      headers: {
...
function initBuffer(projectId, env) {
  apiService = new ApiService({ projectId, env });
  eventBuffer = new EventBuffer(
    { persistenceEnabled: false },
    (snapshot) => apiService.sendEvents(snapshot)
  );
}
